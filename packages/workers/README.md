...
| 2 스크립트 | `script.py` | **Claude Code 스킬(shorts-script)** | `claude` CLI(Claude Code) — **API 키 불필요** | P2-4 |
...
Anthropic API 키가 아니라 **Claude Code 스킬 `shorts-script`** (`.claude/skills/shorts-script/SKILL.md`)로 작성한다.
워커는 `claude -p`(헤드리스)로 호출 → 상품 데이터 → 훅·씬·자막·CTA JSON. `build_prompt`/`parse_script`는 결정적(단위테스트).

## 상태 (P2-4·5·7)
- **코드 구현 완료** + **결정적 로직 단위테스트**: 프롬프트 구성·응답 파싱(script), TTS 요청 구성(voice), 추출→productSchema 정규화(scrape).
...
# script: Claude Code 설치·로그인 필요 (claude CLI)
```
